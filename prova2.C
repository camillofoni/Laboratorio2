...
void prova2() {
  ...
  TH1F *h2 = new TH1F("h2", "angolo azimutale", 1000, 0, 2 * M_PI);
  TH1F *h3 = new TH1F("h3", "angolo polare", 1000, 0, M_PI);
  TH1F *h4 = new TH1F("h4", "impulso", 500, 0, 7);
  TH1F *h5 = new TH1F("h5", "impulso trasverso", 500, 0, 5);
  TH1F *h6 = new TH1F("h6", "energia delle particelle", 500, 0, 7);
  TH1F *h7 = new TH1F("h7", "tutte cariche", 50, 0, 3);
  h7->Sumw2();
  TH1F *h8 = new TH1F("h8", "cariche discordi", 100, 0, 10);
  h8->Sumw2();
  TH1F *h9 = new TH1F("h9", " cariche concordi", 100, 0, 10);
  h9->Sumw2();
  TH1F *h10 = new TH1F("h10", "pioni e kaoni discordi", 100, 0, 10);
  h10->Sumw2();
  TH1F *h11 = new TH1F("h11", "pioni e kaoni concordi", 100, 0, 3);
  h11->Sumw2();
  ...
  for (int j = 0; j < 1e5; ++j) {
    ...
    for (int i = 0; i < 100; ++i) {
      ...
      } else  // k*
      {
        ...
        h7->Fill(EventParticles[100 + decaycounter].InvMass(
            EventParticles[101 + decaycounter]));
        decaycounter++;*/
      }
    ...
    /*for (int i = 0; i < eventParticlesSize; ++i) {
      ...
      if (firstIndex == 1 || firstIndex == 3 || firstIndex == 5) {
        ...
        for (int j = 0; j < eventParticlesSize; ++j) {
          ...
          } else if (secondIndex == 0 || secondIndex == 2 || secondIndex == 4) {
            h8->Fill(EventParticles[firstIndex].InvMass(
                EventParticles[secondIndex]));
          }
        }
